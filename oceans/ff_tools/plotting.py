...
# created:  09-Sep-2011
# modified: Fri 09 Sep 2011 03:08:49 PM EDT
#
...
def etopo_subset(min_lat,max_lat,min_lon,max_lon, tfile=None, smoo=False):
    ...
    bathy = etopo.variables["z"][int(res[2]):int(res[3]),int(res[0]):int(res[1])]
    # using from the grid above
    #lon = etopo.variables["x"][int(res[2]):int(res[3]),int(res[0]):int(res[1])]
    #lat = etopo.variables["y"][int(res[2]):int(res[3]),int(res[0]):int(res[1])]
